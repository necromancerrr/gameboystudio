 ...
 * architecture.
 *
 * Two extensions to the core contract that GBA forced, both now in
 * src/emulation/core/types.ts:
 *   - L and R shoulder buttons, so LogicalButton is ten wide, not eight.
 *   - 240x160, so screen dimensions come from SCREEN_SIZE, not a constant.
 *
 * See D-026 for the integration decisions, GBA_SPIKE.md for what proving this
 * core cost.
 */
...
export const GBA_SCREEN_WIDTH = SCREEN_SIZE.GBA.width;
export const GBA_SCREEN_HEIGHT = SCREEN_SIZE.GBA.height;

/** mGBA's own button names, which are not ours. */
const BUTTON_NAMES: Record<LogicalButton, string> = {
  up: 'Up',
  down: 'Down',
  left: 'Left',
  right: 'Right',
...
export interface GbaAdapterOptions {
...
export class GbaAdapter implements EmulatorAdapter {
  ...
  constructor(options: GbaAdapterOptions) {
  ...
  async loadGame(source: GameSource): Promise<void> {
    const [core, rom] = await Promise.all([
    ...
    core.FS.writeFile(this.romPath, rom);

    if (!core.loadGame(this.romPath)) {
      throw new Error('mGBA rejected the ROM — not a valid Game Boy Advance image');
    }

    /**
     * Loaded, then immediately paused. mGBA starts running the moment a ROM
     * loads; the player decides when a game begins — on a phone that is the
     * Play tap, which is also the gesture browsers require before audio can
     * start. Handing back a running game would take that decision away.
     */
    core.pauseGame();
    this.running = false;
  ...
  async sampleScreen(): Promise<{ colours: number; hash: number } | null> {
    const core = this.module;
    if (!core || this.destroyed) return null;

    const name = `probe-${(this.probeSeq += 1)}.png`;
    const path = `${core.filePaths().screenshotsPath}/${name}`;
    if (!core.screenshot(name)) return null;
     ...
     */
    const png = Uint8Array.from(core.FS.readFile(path));
  ...
  async reset(): Promise<void> {
    ...
    // quickReload leaves the core running whatever it was doing before.
    this.running = true;
    if (this.muted) this.module.pauseAudio();
  }
  ...
  loadSave(data: Uint8Array): boolean {
    ...
    /**
     * mGBA reads the save file when the ROM is loaded and offers no way to
     * inject one into a running core, so applying a save means reloading. The
     * player calls this once, before start(), so nothing visible restarts.
     */
    this.module.quickReload();
    if (this.running) this.module.pauseGame();
    this.running = false;
    return true;
  }

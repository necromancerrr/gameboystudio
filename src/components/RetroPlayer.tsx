...
/**
 * Thin React shell around an EmulatorAdapter. The component owns mounting,
 * layout and teardown; it does not own the run loop, and it does not know which
 * core is behind the adapter.
 *
 * Which core it gets is decided by `source.console` and nothing else. That is
 * the D-005 boundary doing its job: adding Game Boy Advance added a line to a
 * switch, not a second player.
 *
...
import { GbaAdapter } from '@/emulation/gba/GbaAdapter';
import {
  SCREEN_SIZE,
  hasShoulders,
  screenAspect,
...
import { readSaveAsync, writeSaveAsync } from '@/emulation/core/saveStorage';
...
/**
 * The only place in the UI that names a core.
 *
 * Both adapters satisfy EmulatorAdapter; they differ in what they need told.
 * binjgb reports battery writes, mGBA reports save-data writes, and the two
 * callbacks mean the same thing to this component.
 */
function createAdapter(console: ConsoleId, hooks: AdapterHooks): EmulatorAdapter {
  if (console === 'GBA') {
    return new GbaAdapter({
      canvas: hooks.canvas,
      onError: hooks.onError,
      onSaveDirty: hooks.onSaveDirty,
      onFirstFrame: hooks.onFirstFrame,
    });
  }
  return new BinjgbAdapter({
    canvas: hooks.canvas,
    onError: hooks.onError,
    onBatteryDirty: hooks.onSaveDirty,
    onFirstFrame: hooks.onFirstFrame,
  });
}
...
export function RetroPlayer({ source, title }: RetroPlayerProps) {
  ...
  const shoulders = hasShoulders(consoleId);
  ...
  useEffect(() => {
    ...
    const flushSave = () => {
      ...
      // Fire and forget: a GBA save goes to IndexedDB and cannot be awaited on
      // the pagehide path anyway. The write is issued before teardown either
      // way, which is what matters.
      void writeSaveAsync(saveKey, data);
    };

    const adapter = createAdapter(consoleId, {
      canvas,
      onError: (error) => {
        if (cancelled) return;
        setStatus('error');
  ...
  const screen = SCREEN_SIZE[consoleId];
  // One custom property drives both layouts, so the canvas keeps its ratio
  // whether it is in the desktop frame or filling a phone in landscape.
  const aspectStyle = { '--gbs-screen-aspect': screenAspect(consoleId) } as CSSProperties;
    ...
    >
      ...
      {handheldActive ? (
        <div className="handheld__deck" data-hidden={showTouchDeck ? 'false' : 'true'}>
          {showTouchDeck ? (
            <TouchControls
              onButton={handleTouchButton}
              onReleaseAll={releaseTouch}
              shoulders={shoulders}
            />
          ) : null}
        </div>

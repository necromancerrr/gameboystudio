...
exports.runHostedGame = runHostedGame;
...
function runHostedGame(game) {
    ...
    window.addEventListener('message', (event) => {
        ...
        if (event.source !== window.parent)
            return;
        const message = (0, protocol_1.parseHostMessage)(event.data);
        if (message)
            void apply(message);
    });
    /**
     ...
     */
    const hello = () => send({ t: 'hello', v: protocol_1.FRAME_PROTOCOL_VERSION, width: game.width, height: game.height });
    hello();

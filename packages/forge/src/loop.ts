...
import { defaultGenerator, type GameGenerator, type Proposal } from './generator.js';
...
async function runRevision(
  ...
  options: LoopOptions,
  /**
   * Already generated by the caller. `forgeNew` has to see the proposal before
   * it can name the project, and with a real model behind the boundary asking
   * twice is a second full generation — paid for, slower, and not even
   * guaranteed to produce the same game.
   */
  made: { proposal: Proposal; ms: number },
  kind: 'new' | 'change',
): Promise<LoopResult> {
...
export async function forgeNew(request: string, options: LoopOptions): Promise<LoopResult> {
  ...
  const started = Date.now();
  const proposal = await generate(generator, options, request);
  const made = { proposal, ms: Date.now() - started };

  const slug = slugify(proposal.spec.title);
  const project = Project.create(options.root, options.id ?? projectId(slug), slug);
  return runRevision(project, request, generator, options, made, 'new');
}
...
export async function forgeRevise(
  ...
  return runRevision(
    ...
    generator,
    options,
    { proposal, ms: Date.now() - started },
    'change',
  );
}

/**
 * Generation, with the failure recorded.
 *
 * A generator that throws never reaches `runRevision`, so without this the one
 * category of failure the log most needs — the request that could not even be
 * attempted — would be the one category it never saw.
 */
async function generate(
  ...
  try {
    return await generator.propose(request, previous);
  } catch (error) {
    logRequest(options.root, {
      at: new Date().toISOString(),
      projectId: options.id ?? '(none)',
      revision: 0,
      kind: previous ? 'change' : 'new',
      request,
      generator: generator.name,
      ok: false,
      applied: [],
